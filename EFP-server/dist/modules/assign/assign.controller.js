"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AssignController = void 0;
const common_1 = require("@nestjs/common");
const assign_service_1 = require("./assign.service");
const update_assign_dto_1 = require("./dto/update-assign.dto");
const getList_assign_dto_1 = require("./dto/getList-assign.dto");
const mail_service_1 = require("../mail/mail.service");
let AssignController = class AssignController {
    constructor(assignService, mailService) {
        this.assignService = assignService;
        this.mailService = mailService;
    }
    create(createAssignDto) {
        return this.assignService.assignEmployeeToProject(createAssignDto);
    }
    findAll(params) {
        return this.assignService.getAssigns(params);
    }
    findOne(id) {
        return this.assignService.getAssignById(id);
    }
    update(id, updateAssignDto) {
        return this.assignService.update(id, updateAssignDto);
    }
    remove(data) {
        return this.assignService.remove(data);
    }
};
exports.AssignController = AssignController;
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Array]),
    __metadata("design:returntype", void 0)
], AssignController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(),
    __param(0, (0, common_1.Query)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [getList_assign_dto_1.GetAssignParams]),
    __metadata("design:returntype", void 0)
], AssignController.prototype, "findAll", null);
__decorate([
    (0, common_1.Get)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], AssignController.prototype, "findOne", null);
__decorate([
    (0, common_1.Patch)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, update_assign_dto_1.UpdateAssignDto]),
    __metadata("design:returntype", void 0)
], AssignController.prototype, "update", null);
__decorate([
    (0, common_1.Delete)(),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], AssignController.prototype, "remove", null);
exports.AssignController = AssignController = __decorate([
    (0, common_1.Controller)('assign'),
    __metadata("design:paramtypes", [assign_service_1.AssignService,
        mail_service_1.MailService])
], AssignController);
//# sourceMappingURL=assign.controller.js.map